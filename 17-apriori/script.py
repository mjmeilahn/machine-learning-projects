...
def inspect (results):
    ...
    return list(zip(leftHandSide, rightHandSide, lifts, confidences, supports,))

# Organize with a Data Frame
resultsInDataFrame = pd.DataFrame(inspect(results), columns=['Left Hand Side', 'Right Hand Side', 'Lift', 'Confidence', 'Support'])

# Sort by Highest Lift
associations = resultsInDataFrame.nlargest(n=10, columns='Lift')
